...
def net_measures(net_fns, dataset, g_fns, n_epochs, lr, subkey, stop_mse=0, print_every=None, compute_acc=False, batch_size=None, loss_fn='mse'):
  ...
  t0 = time.time()
  net_results = net_predictions(net_fns, dataset, n_epochs, lr, subkey, stop_mse=stop_mse, print_every=print_every, compute_acc=compute_acc, batch_size=batch_size, loss_fn=loss_fn)
  t = time.time() - t0
...
def learning_measure_statistics(net_fns, domain, n, f_terms=None, g_terms=[], pred_type='both', n_trials=1, **kwargs):
  ...
  for i in range(n_trials):
    ...
    elif domain in ['mnist', 'fmnist', 'cifar10', 'cifar100']:
    ...
    elif domain in ['mnist normalized', 'fmnist normalized', 'cifar10 normalized', 'cifar100 normalized']:
